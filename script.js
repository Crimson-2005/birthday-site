...
let time = 5;
let heartScore = 0, heartInterval, fallingWordInterval;
...
function startBalloonGame(){
  ...
  for(let i=0;i<3;i++){
    ...
    balloon.classList.add("choice-box"); balloon.innerText="🎈";
...
function startBoxGame(){
  ...
  const box=document.getElementById("gameBox");
  box.innerHTML=""; document.getElementById("gameMessage").innerText="Good words click karo! Wrong pe mat click karna 😌";
  ...
  const scoreDisplay=document.createElement("div"); scoreDisplay.id="scoreDisplay"; scoreDisplay.innerText="Score: 0";
  const gameArea=document.createElement("div"); gameArea.id="catchGameArea";
  box.appendChild(scoreDisplay); box.appendChild(gameArea);
  fallingWordInterval=setInterval(()=>{
    if(!gameActive) return;
    const word=document.createElement("div"); word.classList.add("falling-word");
    const isGood=Math.random()>0.4; word.innerText=isGood ? goodWords[Math.floor(Math.random()*goodWords.length)] : badWords[Math.floor(Math.random()*badWords.length)];
    word.dataset.good=isGood; word.style.left=Math.random()*280+"px"; gameArea.appendChild(word);
    let topPos=0;
    const fallInterval=setInterval(()=>{
      if(!gameActive){ clearInterval(fallInterval); return; }
      topPos+=0.5; word.style.top=topPos+"px";
      if(topPos>260){ word.remove(); clearInterval(fallInterval); }
    },30);
    word.onclick=function(){
      if(!gameActive) return;
      if(word.dataset.good==="true") score++; else score=Math.max(0,score-1);
      scoreDisplay.innerText="Score: "+score; word.remove(); clearInterval(fallInterval);
      if(score>=10){ gameActive=false; clearInterval(fallingWordInterval); setTimeout(()=>unlockMainContent(),800);}
    }
  },1000);
}
...
function unlockMainContent(){
  ...
  document.getElementById("gameIntro").style.display="none";
  mainContent.style.display="block";
  setTimeout(()=>{ mainContent.style.opacity="1"; mainContent.style.pointerEvents="auto"; },100);
